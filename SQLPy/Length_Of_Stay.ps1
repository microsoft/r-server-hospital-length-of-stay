...
##########################################################################
   try
    ...
    Write-Host -ForeGroundColor 'Cyan' (" Training Gradient Boosted Trees (rx_fast_trees implementation)...")
    $modelName = 'FT'
    ...
    # execute the scoring 
    Write-Host -ForeGroundColor 'Cyan' (" Scoring Gradient Boosted Trees (rx_fast_trees implementation)...")
    $query = "EXEC score $modelName, 'SELECT * FROM LoS WHERE eid NOT IN (SELECT eid FROM Train_Id)', 'Fast_Prediction'"
    ExecuteSQLQuery $query
    ...
    Write-Host -ForeGroundColor 'Cyan' (" Evaluating Gradient Boosted Trees (rx_fast_trees implementation) ...")
...
##########################################################################
   try
    ...
    Write-Host -ForeGroundColor 'Cyan' (" Scoring Gradient Boosted Trees (rx_fast_trees implementation)...")
    $modelName = 'FT'
    $query = "EXEC score $modelName, 'SELECT * FROM LoS_Prod', 'Fast_Prediction_Prod'"
    ExecuteSQLQuery $query
...
if ($ans -eq 'y' -or $ans -eq 'Y')
{
    ...
    # execute the training 
    $ans2 = Read-Host 'Train Random Forest (rx_dforest implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    ...
    {
    Write-Host -ForeGroundColor 'Cyan' (" Training Random Forest (rx_dforest implementation) ...")
    $modelName = 'RF'
    $query = "EXEC train_model $modelName, $output1"
    ExecuteSQLQuery $query
    }

    $ans3 = Read-Host 'Train Gradient Boosted Trees (rx_btrees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    ...
    {
    Write-Host -ForeGroundColor 'Cyan' (" Training Gradient Boosted Trees (rx_btrees implementation)  ...")
    $modelName = 'GBT'
    $query = "EXEC train_model $modelName, $output1"
    ExecuteSQLQuery $query
    }

    $ans4 = Read-Host 'Train Gradient Boosted Trees (rx_fast_trees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans4 -eq 'E' -or $ans4 -eq 'e')
    {
        return
    } 
    ...
    $ans5 = Read-Host 'Train Neural Network (rx_neural_network implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans5 -eq 'E' -or $ans5 -eq 'e')
    {
        return
    } 
    if ($ans5 -eq 'y' -or $ans5 -eq 'Y')
    {
    Write-Host -ForeGroundColor 'Cyan' (" Training Nerual Network (rx_neural_network implementation)  ...")
    $modelName = 'NN'
    $query = "EXEC train_model $modelName, $output1"
    ExecuteSQLQuery $query
    }
...
if ($ans -eq 'y' -or $ans -eq 'Y')
{
    ...
    # execute the scoring
    $ans2 = Read-Host 'Score Random Forest (rx_dforest implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans2 -eq 'E' -or $ans2 -eq 'e')
    {
        return
    } 
    if ($ans2 -eq 'y' -or $ans2 -eq 'Y')
    {
        ...
        Write-Host -ForeGroundColor 'Cyan' (" Scoring Random Forest (rx_dforest implementation) ...")
        $modelName = 'RF'    
    ...
    $ans3 = Read-Host 'Score Boosted Trees (rx_btrees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans3 -eq 'E' -or $ans3 -eq 'e')
    {
        return
    } 
    if ($ans3 -eq 'y' -or $ans3 -eq 'Y')
    {
        ...
        Write-Host -ForeGroundColor 'Cyan' (" Scoring Boosted Trees (rx_btrees implementation) ...")
        $modelName = 'GBT'
    ...
    $ans4 = Read-Host 'Score Gradient Boosted Trees (rx_fast_trees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans4 -eq 'E' -or $ans4 -eq 'e')
    {
        return
    } 
    if ($ans4 -eq 'y' -or $ans4 -eq 'Y')
    {
        $output4 = Read-Host 'Output table name holding predictions? Type D or d for default (rx_fast_trees)'
        if ($output4 -eq 'D' -or $output4 -eq 'd')
        {
        $output4 = 'Fast_Prediction'
        }

        Write-Host -ForeGroundColor 'Cyan' (" Scoring Gradient Boosted Trees (rx_fast_trees implementation) ...")
        $modelName = 'FT'    
        $query = "EXEC score $modelName, 'SELECT * FROM $output1 WHERE eid NOT IN (SELECT eid FROM Train_Id)', $output4"
        ExecuteSQLQuery $query
         }

    if ($ans4 -eq 's' -or $ans4 -eq 'S')
    {
    $output4 = 'Fast_Prediction'
    }


    $ans5 = Read-Host 'Score Neural Network (rx_neural_network implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans5 -eq 'E' -or $ans5 -eq 'e')
    {
        return
    } 
    if ($ans5 -eq 'y' -or $ans5 -eq 'Y')
    {
        $output5 = Read-Host 'Output table name holding predictions? Type D or d for default (NN_Prediction)'
        if ($output5 -eq 'D' -or $output5 -eq 'd')
        {
        $output5 = 'NN_Prediction'
        }

        Write-Host -ForeGroundColor 'Cyan' (" Scoring Neural Network (rx_neural_network implementation) ...")
        $modelName = 'BB'
        $query = "EXEC score $modelName,'SELECT * FROM $output1 WHERE eid NOT IN (SELECT eid FROM Train_Id)', $output5"
        ExecuteSQLQuery $query
         }

    if ($ans5 -eq 's' -or $ans5 -eq 'S')
    {
    $output5 = 'NN_Prediction'
    }
}
...
if ($ans -eq 'y' -or $ans -eq 'Y')
{
    ...
    # execute the evaluation
    $ans2 = Read-Host 'Evaluate Random Forest (rx_dforest implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans2 -eq 'E' -or $ans2 -eq 'e')
    {
        return
    } 
    if ($ans2 -eq 'y' -or $ans2 -eq 'Y')
    {
        Write-Host -ForeGroundColor 'Cyan' (" Evaluating Random Forest (rx_dforest implementation) ...")
        $modelName = 'RF'
        $query = "EXEC evaluate $modelName, $output2"
        ExecuteSQLQuery $query
    }

    $ans3 = Read-Host 'Evaluate Gradient Boosted Trees (rx_btrees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans3 -eq 'E' -or $ans3 -eq 'e')
    {
        return
    } 
    if ($ans3 -eq 'y' -or $ans3 -eq 'Y')
    {
        Write-Host -ForeGroundColor 'Cyan' (" Evaluating Boosted Trees (rx_btrees implementation) ...")
        $modelName = 'GBT'
        $query = "EXEC evaluate $modelName, $output3"
        ExecuteSQLQuery $query
    }

    $ans4 = Read-Host 'Evaluate Gradient Boosted Trees (rx_fast_trees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans4 -eq 'E' -or $ans4 -eq 'e')
    {
        return
    } 
    if ($ans4 -eq 'y' -or $ans4 -eq 'Y')
    {
        Write-Host -ForeGroundColor 'Cyan' (" Evaluating Boosted Trees (rx_fast_trees implementation) ...")
        $modelName = 'FT'
        $query = "EXEC evaluate $modelName, $output4"
        ExecuteSQLQuery $query
    }

    $ans5 = Read-Host 'Evaluate Neural Network (rx_neural_network implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans5 -eq 'E' -or $ans5 -eq 'e')
    {
        return
    } 
    if ($ans5 -eq 'y' -or $ans5 -eq 'Y')
    {
        Write-Host -ForeGroundColor 'Cyan' (" Evaluating Neural Network (rx_neural_network implementation) ...")
        $modelName = 'FT'
        $query = "EXEC evaluate $modelName, $output5"
        ExecuteSQLQuery $query
    }
...
if ($ans -eq 'y' -or $ans -eq 'Y')
{
  # create the stored procedure for predicting 
    ...
    # execute the scoring
    $ans2 = Read-Host 'Use Random Forest (rx_dforest implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans2 -eq 'E' -or $ans2 -eq 'e')
    {
        return
    } 
    ...
    $ans3 = Read-Host 'Use Boosted Trees (rx_btrees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans3 -eq 'E' -or $ans3 -eq 'e')
    {
        return
    } 
    ...
    $ans4 = Read-Host 'Use Gradient Boosted Trees (rx_fast_trees implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans4 -eq 'E' -or $ans4 -eq 'e')
    {
        return
    } 
    if ($ans4 -eq 'y' -or $ans4 -eq 'Y')
    {
        $output4 = Read-Host 'Output table name holding predictions? Type D or d for default (Fast_Prediction_Prod)'
        if ($output4 -eq 'D' -or $output4 -eq 'd')
        {
        $output4 = 'Fast_Prediction_Prod'
        }

        Write-Host -ForeGroundColor 'Cyan' (" Scoring Gradient Boosted Trees (rx_fast_trees implementation) ...")
        $modelName = 'FT'
        $query = "EXEC score $modelName, 'SELECT * FROM $output1', $output4"
        ExecuteSQLQuery $query
        }

    $ans5 = Read-Host 'Use Neural Network (rx_neural_network implementation): Yes [y|Y], Exit [e|E], Skip [s|S]?'
    if ($ans5 -eq 'E' -or $ans5 -eq 'e')
    {
        return
    } 
    if ($ans5 -eq 'y' -or $ans5 -eq 'Y')
    {
        $output5 = Read-Host 'Output table name holding predictions? Type D or d for default (NN_Prediction_Prod)'
        if ($output5 -eq 'D' -or $output5 -eq 'd')
        {
        $output5 = 'NN_Prediction_Prod'
        }

        Write-Host -ForeGroundColor 'Cyan' (" Scoring Neural Network (rx_neural_network implementation) ...")
        $modelName = 'NN'
        $query = "EXEC score $modelName, 'SELECT * FROM $output1', $output5"
        ExecuteSQLQuery $query
        }
